...
export function AdminPage() {
  ...
  return <section><div className="page-heading"><div><p className="eyebrow">Administration</p><h1>Quản trị hệ thống</h1></div></div>
    ...
    {tab === "staff" && <form className="panel form-grid" onSubmit={createStaff}><FormField label="Tên đăng nhập"><input value={username} onChange={(e) => setUsername(e.target.value)} /></FormField><FormField label="Mật khẩu"><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></FormField><FormField label="Vai trò"><select value={staffRole} onChange={(e) => setStaffRole(e.target.value)}><option value="staff">Nhân viên</option><option value="admin">Quản trị viên</option></select></FormField><div className="form-actions"><button disabled={busy}>Tạo tài khoản</button></div></form>}
